...
if __name__ == "__main__":
    ...
    if args.strategy == "vector":
        ...
        context = query_milvus(args.database_path, args.embedding_model, args.query)

    elif args.strategy == "kw":
        print("Using keyword-based retrieval.")
        keywords = args.query.split()
        context = query_whoosh(args.database_path, keywords)

    else:
        raise ValueError(f"Unknown strategy: {args.strategy}")

    response = query_llm(args.generation_model, context, args.query)
    print("\n=== Final Answer ===")
    print(response)
